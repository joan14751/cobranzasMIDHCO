...
import { Brain, ShieldCheck, AlertTriangle, HelpCircle, Search, FileText } from 'lucide-react'
...
// === SUBCOMPONENTE: SKELETON LOADER RESPONSIVO ===
function MlCardSkeleton() {
  return (
    <div className="rounded-2xl border border-gray-100 bg-white p-4 md:p-5 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4 animate-pulse">
      {/* Información principal simulada */}
      ...
      {/* Bloque de indicadores derecho simulado */}
      <div className="flex flex-row md:flex-col items-center md:items-end justify-between md:justify-center gap-3 pt-3 md:pt-0 border-t md:border-t-0 border-gray-50 min-w-[140px]">
        {/* Badge de Riesgo */}
        <div className="h-6 w-24 bg-slate-100 rounded-full" />
        {/* Porcentaje de Probabilidad */}
        <div className="text-right flex flex-col items-end md:items-end gap-1">
    ...
    <div className="space-y-6 p-4 md:p-6">
      <div>
        <h1 className="text-xl md:text-2xl font-bold text-gray-800 flex items-center gap-2">
          <Brain className="h-6 w-6 text-blue-500" /> Gestión Inteligente de Cobranza MIDHCO (ML) 
        </h1>
      ...
      {/* MANEJO DE ESTADOS DE CARGA / ERROR / LISTADO */}
      {error ? (
        <div className="rounded-2xl border border-red-200 bg-red-50 p-4 text-sm text-red-600">{error}</div>
      ) : (
        <div className="grid grid-cols-1 gap-4">
          {loading ? (
            /* CAMBIO CLAVE: Renderiza 4 tarjetas fantasma simulando la lista */
            <>
              <MlCardSkeleton />
              <MlCardSkeleton />
              <MlCardSkeleton />
            ...
            filteredRecommendations.map((item, index) => {
              ...
              return (
                <div key={item.id || index} className="rounded-2xl border border-gray-100 bg-white p-4 md:p-5 shadow-sm transition hover:shadow-md flex flex-col md:flex-row md:items-center justify-between gap-4">
                  ...
                  <div className="flex flex-row md:flex-col items-center md:items-end justify-between md:justify-center gap-2 min-w-[140px] pt-3 md:pt-0 border-t md:border-t-0 border-gray-100">
